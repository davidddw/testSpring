...
public class VmwareClient {
	...
	public void connect(String url, String userName, String password)
		...
		ServiceContent serviceContent = vimPort
				.retrieveServiceContent(svcInstRef);
	...
	public boolean validate() {
		...
		try {
			List<ObjectContent> ocary = vimPort.retrieveProperties(
					getPropCol(), specArr);
	...
	public <T> T getDynamicProperty(ManagedObjectReference mor,
		...
		if (objContent != null && objContent.size() > 0) {
			List<DynamicProperty> dynamicProperty = objContent.get(0)
					.getPropSet();
	...
	private List<ObjectContent> retrieveMoRefProperties(
			ManagedObjectReference mObj, List<String> props) throws Exception {
		PropertySpec pSpec = new PropertySpec();
		pSpec.setAll(false);
	...
	public boolean waitForTask(ManagedObjectReference task)
			...
			InvalidCollectorVersionFaultMsg, Exception {
	...
	public ManagedObjectReference getDecendentMoRef(
		...
		List<ObjectContent> ocary = vimPort.retrieveProperties(getPropCol(),
				specArr);
