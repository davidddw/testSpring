...
import com.yunshan.cloudstack.utils.ActionDelegate;
import com.yunshan.cloudstack.utils.DatastoreFile;
import com.yunshan.cloudstack.utils.Pair;
import com.yunshan.cloudstack.utils.Ternary;
import com.yunshan.cloudstack.vmware.util.VmwareContext;
import com.yunshan.cloudstack.vmware.util.VmwareHelper;
import com.yunshan.cloudstack.utils.concurrency.NamedThreadFactory;
import com.yunshan.cloudstack.utils.script.Script;
import com.yunshan.cloudstack.vmware.mo.SnapshotDescriptor.SnapshotInfo;

public class VirtualMachineMO extends BaseMO {
	private static final Logger s_logger = Logger
			.getLogger(VirtualMachineMO.class);
	private static final ExecutorService MonitorServiceExecutor = Executors
			.newCachedThreadPool(new NamedThreadFactory("VM-Question-Monitor"));
	...
	public VirtualMachineMO(VmwareContext context, String morType,
			String morValue) {
	...
	public Pair<DatastoreMO, String> getOwnerDatastore(String dsFullPath)
			throws Exception {
		...
		if (ocs != null) {
			for (ObjectContent oc : ocs) {
				...
				if (prop.getVal().toString().equals(dsName)) {
					return new Pair<DatastoreMO, String>(new DatastoreMO(
							_context, oc.getObj()), dsName);
				}
			}
		}
	...
	public String getVmName() throws Exception {
		return (String) getContext().getVimClient().getDynamicProperty(_mor,
				"name");
	}

	public GuestInfo getVmGuestInfo() throws Exception {
		return (GuestInfo) getContext().getVimClient().getDynamicProperty(_mor,
				"guest");
	}
	...
	public boolean isVMwareToolsRunning() throws Exception {
		...
		if (guestInfo != null) {
			if ("guestToolsRunning".equalsIgnoreCase(guestInfo
	...
	public boolean powerOn() throws Exception {
		...
		ManagedObjectReference morTask = _context.getService().powerOnVMTask(
				_mor, null);
		...
		Future<?> future = MonitorServiceExecutor.submit(new Runnable() {
			public void run() {
				...
				while (!flags[0]) {
					try {
						VirtualMachineRuntimeInfo runtimeInfo = vmMo
								.getRuntimeInfo();
						VirtualMachineQuestionInfo question = runtimeInfo
								.getQuestion();
						if (question != null) {
							s_logger.info("Question id: " + question.getId());
							s_logger.info("Question text: "
									+ question.getText());
							if (question.getMessage() != null) {
								for (VirtualMachineMessage msg : question
										.getMessage()) {
									if (s_logger.isInfoEnabled()) {
										s_logger.info("msg id: " + msg.getId());
										s_logger.info("msg text: "
												+ msg.getText());
									}
									if ("msg.uuid.altered".equalsIgnoreCase(msg
											.getId())) {
										s_logger.info("Found that VM has a pending question that we need to answer programmatically, question id: "
												+ msg.getId()
							...
							if (choice != null) {
								for (ElementDescription info : choice
										.getChoiceInfo()) {
									if (s_logger.isTraceEnabled()) {
	...
	public boolean safePowerOff(int shutdownWaitMs) throws Exception {
		...
		if (isVMwareToolsRunning()) {
			try {
				...
				if (getResetSafePowerState() != VirtualMachinePowerState.POWERED_OFF) {
					s_logger.info("can not gracefully shutdown VM within "
							+ (shutdownWaitMs / 1000)
							+ " seconds, we will perform force power off on VM "
							+ vmName);
					return powerOffNoCheck();
				}
				...
				s_logger.warn("Failed to do guest-os graceful shutdown due to "
	...
	private boolean powerOffNoCheck() throws Exception {
		ManagedObjectReference morTask = _context.getService().powerOffVMTask(
				_mor);
		...
		if (result) {
			...
			s_logger.error("VMware powerOffVM_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
	...
	public VirtualMachinePowerState getResetSafePowerState() throws Exception {
		...
		for (int i = 0; i < 3; i++) {
			powerState = (VirtualMachinePowerState) getContext().getVimClient()
					.getDynamicProperty(_mor, "runtime.powerState");
	...
	public VirtualMachinePowerState getPowerState() throws Exception {
		return (VirtualMachinePowerState) getContext().getVimClient()
				.getDynamicProperty(_mor, "runtime.powerState");
	}
	...
	public boolean reset() throws Exception {
		...
		if (result) {
			...
			s_logger.error("VMware resetVM_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
		return false;
	}
	...
	public boolean migrate(ManagedObjectReference morRp,
			ManagedObjectReference morTargetHost) throws Exception {
		ManagedObjectReference morTask = _context.getService().migrateVMTask(
				_mor, morRp, morTargetHost,
				VirtualMachineMovePriority.DEFAULT_PRIORITY, null);
		...
		if (result) {
			...
			s_logger.error("VMware migrateVM_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
	...
	public boolean changeDatastore(VirtualMachineRelocateSpec relocateSpec)
		...
		ManagedObjectReference morTask = _context
				.getVimClient()
				.getService()
				.relocateVMTask(_mor, relocateSpec,
	...
	public boolean changeHost(VirtualMachineRelocateSpec relocateSpec)
		...
		if (result) {
			...
			s_logger.error("VMware RelocateVM_Task to change host failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
		return false;
	}

	public boolean relocate(ManagedObjectReference morTargetHost)
			throws Exception {
		VirtualMachineRelocateSpec relocateSpec = new VirtualMachineRelocateSpec();
		relocateSpec.setHost(morTargetHost);

		ManagedObjectReference morTask = _context.getService().relocateVMTask(
				_mor, relocateSpec, null);
		...
		if (result) {
			...
			s_logger.error("VMware relocateVM_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
	...
	public VirtualMachineSnapshotInfo getSnapshotInfo() throws Exception {
		return (VirtualMachineSnapshotInfo) _context.getVimClient()
				.getDynamicProperty(_mor, "snapshot");
	}

	public boolean createSnapshot(String snapshotName,
			String snapshotDescription, boolean dumpMemory, boolean quiesce)
			throws Exception {

		long apiTimeout = _context.getVimClient().getVcenterSessionTimeout();
		ManagedObjectReference morTask = _context.getService()
				.createSnapshotTask(_mor, snapshotName, snapshotDescription,
						dumpMemory, quiesce);
		...
		if (result) {
			...
			if (morSnapshot == null) {
				s_logger.error("We've been waiting for over "
						+ apiTimeout
						+ " milli seconds for snapshot MOR to be appearing in vCenter after CreateSnapshot task is done, but it is still not there?!");
				return false;
			}
			s_logger.debug("Waited for "
					+ (System.currentTimeMillis() - startTick)
					+ " seconds for snapshot object [" + snapshotName
					+ "] to appear in vCenter.");
			return true;
		} else {
			s_logger.error("VMware createSnapshot_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
	...
	public boolean removeSnapshot(String snapshotName, boolean removeChildren)
			throws Exception {
		...
		ManagedObjectReference morTask = _context.getService()
				.removeSnapshotTask(morSnapshot, removeChildren, true);
		boolean result = _context.getVimClient().waitForTask(morTask);
		if (result) {
			_context.waitForTaskProgressDone(morTask);
			return true;
		} else {
			s_logger.error("VMware removeSnapshot_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
	...
	public boolean removeAllSnapshots() throws Exception {
		...
		if (snapshotInfo != null && snapshotInfo.getRootSnapshotList() != null) {
			List<VirtualMachineSnapshotTree> tree = snapshotInfo
					.getRootSnapshotList();
			for (VirtualMachineSnapshotTree treeNode : tree) {
				ManagedObjectReference morTask = _context.getService()
						.removeSnapshotTask(treeNode.getSnapshot(), true, true);
				boolean result = _context.getVimClient().waitForTask(morTask);
				if (result) {
					_context.waitForTaskProgressDone(morTask);
				} else {
		...
		if (snapshotDiskFile.startsWith("/")) {
			for (Pair<ManagedObjectReference, String> mount : datastoreMounts) {
				if (snapshotDiskFile.startsWith(mount.second())) {
					...
					String dsFullPath = String.format("[%s] %s",
							dsMo.getName(), snapshotDiskFile.substring(mount
									.second().length() + 1));
					s_logger.info("Convert snapshot disk file name to datastore path. "
							+ snapshotDiskFile + "->" + dsFullPath);
					return dsFullPath;
				}
			}

			s_logger.info("Convert snapshot disk file name to datastore path. "
					+ snapshotDiskFile + "->" + snapshotDiskFile);
			return snapshotDiskFile;
		} else {

			// snapshot directory string from VirtualMachineFileInfo ends with /
			String dsFullPath = vmFileInfo.getSnapshotDirectory()
					+ snapshotDiskFile;
			s_logger.info("Convert snapshot disk file name to datastore path. "
					+ snapshotDiskFile + "->" + dsFullPath);
			return dsFullPath;
	...
	public ManagedObjectReference getSnapshotMor(String snapshotName)
		...
		if (info != null) {
			List<VirtualMachineSnapshotTree> snapTree = info
					.getRootSnapshotList();
	...
	public boolean createFullClone(String cloneName,
			ManagedObjectReference morFolder,
		...
		ManagedObjectReference morTask = _context.getService().cloneVMTask(
				_mor, morFolder, cloneName, cloneSpec);
		...
		if (result) {
			...
			s_logger.error("VMware cloneVM_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
	...
	public boolean createLinkedClone(String cloneName,
		...
		if (independentDisks.length > 0) {
			...
			for (int i = 0; i < independentDisks.length; i++) {
				...
				loc.setDiskMoveType(VirtualMachineRelocateDiskMoveOptions.MOVE_ALL_DISK_BACKINGS_AND_DISALLOW_SHARING
						.value());
				diskLocator.add(loc);
			}
	...
	public VirtualMachineRuntimeInfo getRuntimeInfo() throws Exception {
		return (VirtualMachineRuntimeInfo) _context.getVimClient()
				.getDynamicProperty(_mor, "runtime");
	}

	public VirtualMachineConfigInfo getConfigInfo() throws Exception {
		return (VirtualMachineConfigInfo) _context.getVimClient()
				.getDynamicProperty(_mor, "config");
	}

	public VirtualMachineConfigSummary getConfigSummary() throws Exception {
		return (VirtualMachineConfigSummary) _context.getVimClient()
				.getDynamicProperty(_mor, "summary.config");
	}

	public VirtualMachineFileInfo getFileInfo() throws Exception {
		return (VirtualMachineFileInfo) _context.getVimClient()
				.getDynamicProperty(_mor, "config.files");
	}
				...
				if (props != null) {
					for (DynamicProperty prop : props) {
						if (prop.getName().equals("layoutEx")) {
							fileLayout = (VirtualMachineFileLayoutEx) prop
									.getVal();
							break;
						}
					}
	...
	public ManagedObjectReference getParentMor() throws Exception {
		return (ManagedObjectReference) _context.getVimClient()
				.getDynamicProperty(_mor, "parent");
	}
	...
	public List<NetworkDetails> getNetworksWithDetails() throws Exception {
		...
		int gcTagKey = getCustomFieldKey("Network",
				CustomFieldConstants.CLOUD_GC);

		if (gcTagKey == 0) {
			gcTagKey = getCustomFieldKey("DistributedVirtualPortgroup",
			...
			for (ObjectContent oc : ocs) {
				...
				for (DynamicProperty prop : oc.getPropSet()) {
					if (prop.getName().equals("name"))
						...
						CustomFieldStringValue val = (CustomFieldStringValue) prop
								.getVal();
						if (val != null)
				...
				NetworkDetails details = new NetworkDetails(name, oc.getObj(),
						(morVms != null ? morVms.getManagedObjectReference()
								.toArray(
										new ManagedObjectReference[morVms
												.getManagedObjectReference()
												.size()]) : null), gcTagValue);

				networks.add(details);
			}
			s_logger.debug("Retrieved " + networks.size()
					+ " networks with key : " + gcTagKey);
		}
	...
	public String getVmxHttpAccessUrl() throws Exception {
		...
		StringBuffer sb = new StringBuffer("https://"
				+ _context.getServerAddress() + "/folder/");
	...
	public boolean setVncConfigInfo(boolean enableVnc, String vncPassword,
			int vncPort, String keyboard) throws Exception {
		VirtualMachineConfigSpec vmConfigSpec = new VirtualMachineConfigSpec();
		OptionValue[] vncOptions = VmwareHelper.composeVncOptions(null,
				enableVnc, vncPassword, vncPort, keyboard);
		vmConfigSpec.getExtraConfig().addAll(Arrays.asList(vncOptions));
		ManagedObjectReference morTask = _context.getService().reconfigVMTask(
				_mor, vmConfigSpec);
		...
		if (result) {
			...
			s_logger.error("VMware reconfigVM_Task failed due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
		return false;
	}

	public boolean configureVm(VirtualMachineConfigSpec vmConfigSpec)
			throws Exception {
		ManagedObjectReference morTask = _context.getService().reconfigVMTask(
				_mor, vmConfigSpec);
	...
	public Pair<String, Integer> getVncPort(String hostNetworkName)
			throws Exception {
		HostMO hostMo = getRunningHost();
		VmwareHypervisorHostNetworkSummary summary = hostMo
				.getHyperHostNetworkSummary(hostNetworkName);
		...
		if (values != null) {
			for (OptionValue option : values) {
				if (option.getKey().equals("RemoteDisplay.vnc.port")) {
					...
					if (value != null) {
						return new Pair<String, Integer>(summary.getHostIp(),
	...
	public void createDisk(String vmdkDatastorePath, int sizeInMb,
		...
		createDisk(vmdkDatastorePath, VirtualDiskType.THIN,
				VirtualDiskMode.PERSISTENT, null, sizeInMb, morDs,
				controllerKey);
	}
	...
	public void createDisk(String vmdkDatastorePath, VirtualDiskType diskType,
		...
		if (s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - createDisk(). target MOR: "
					...
					+ ", diskType: " + diskType + ", diskMode: " + diskMode
					+ ", rdmDeviceName: " + rdmDeviceName + ", datastore: "
					+ morDs.getValue() + ", controllerKey: " + controllerKey);
		...
		if (diskType == VirtualDiskType.THIN
				|| diskType == VirtualDiskType.PREALLOCATED
		...
		} else if (diskType == VirtualDiskType.RDM
				|| diskType == VirtualDiskType.RDMP) {
		...
		if (!result) {
			...
			throw new Exception("Unable to create disk " + vmdkDatastorePath
	...
	public void attachDisk(String[] vmdkDatastorePathChain,
		...
		synchronized (_mor.getValue().intern()) {
			VirtualDevice newDisk = VmwareHelper.prepareDiskDevice(this, null,
					getScsiDeviceControllerKey(), vmdkDatastorePathChain,
					morDs, -1, 1);
			VirtualMachineConfigSpec reConfigSpec = new VirtualMachineConfigSpec();
			VirtualDeviceConfigSpec deviceConfigSpec = new VirtualDeviceConfigSpec();
			...
			ManagedObjectReference morTask = _context.getService()
					.reconfigVMTask(_mor, reConfigSpec);
			boolean result = _context.getVimClient().waitForTask(morTask);
	...
	// vmdkDatastorePath: [datastore name] vmdkFilePath
	public List<Pair<String, ManagedObjectReference>> detachDisk(
			String vmdkDatastorePath, boolean deleteBackingFile)
			throws Exception {

		if (s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - detachDisk(). target MOR: "
					+ _mor.getValue() + ", vmdkDatastorePath: "
		...
		if (!result) {
			...
			throw new Exception("Failed to detach disk due to "
					+ TaskMO.getTaskFailureInfo(_context, morTask));
		}
		_context.waitForTaskProgressDone(morTask);
		...
		if (snapshotDescriptor != null) {
			for (Pair<String, ManagedObjectReference> pair : chain) {
				...
				snapshotDescriptor.removeDiskReferenceFromSnapshot(dsFile
						.getFileName());
			}
			...
			String dsPath = getSnapshotDescriptorDatastorePath();
			assert (dsPath != null);
			String url = getContext().composeDatastoreBrowseUrl(
					dcPair.second(), dsPath);
			getContext().uploadResourceContent(url,
	...
	public void detachAllDisks() throws Exception {
		if (s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - detachAllDisk(). target MOR: "
					+ _mor.getValue());
		...
		if (disks.length > 0) {
			...
			for (int i = 0; i < disks.length; i++) {
				...
				deviceConfigSpecArray[i]
						.setOperation(VirtualDeviceConfigSpecOperation.REMOVE);
			}
			reConfigSpec.getDeviceChange().addAll(
					Arrays.asList(deviceConfigSpecArray));
	...
	public void attachIso(String isoDatastorePath,
			ManagedObjectReference morDs, boolean connect, boolean connectAtBoot)
			throws Exception {

		if (s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - attachIso(). target MOR: "
					+ _mor.getValue() + ", isoDatastorePath: "
					+ isoDatastorePath + ", datastore: " + morDs.getValue()
					+ ", connect: " + connect + ", connectAtBoot: "
					+ connectAtBoot);
	...
	public void detachIso(String isoDatastorePath) throws Exception {
		if (s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - detachIso(). target MOR: "
					+ _mor.getValue() + ", isoDatastorePath: "
					+ isoDatastorePath);
						...
						if (question != null) {
							...
							if (question.getMessage() != null) {
								for (VirtualMachineMessage msg : question
									...
									if ("msg.cdromdisconnect.locked"
											.equalsIgnoreCase(msg.getId())) {
										s_logger.info("Found that VM has a pending question that we need to answer programmatically, question id: "
												+ msg.getId()
		...
		try {
			...
			if (!result) {
				if (s_logger.isDebugEnabled())
					s_logger.trace("vCenter API trace - detachIso() done(failed)");
				throw new Exception("Failed to detachIso due to "
						+ TaskMO.getTaskFailureInfo(_context, morTask));
			}
			_context.waitForTaskProgressDone(morTask);
			s_logger.trace("vCenter API trace - detachIso() done(successfully)");
	...
	public Pair<VmdkFileDescriptor, byte[]> getVmdkFileInfo(
			String vmdkDatastorePath) throws Exception {

		if (s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - getVmdkFileInfo(). target MOR: "
					+ _mor.getValue()
					+ ", vmdkDatastorePath: "
					+ vmdkDatastorePath);
		...
		Pair<VmdkFileDescriptor, byte[]> result = new Pair<VmdkFileDescriptor, byte[]>(
				descriptor, content);
		if (s_logger.isTraceEnabled()) {
			s_logger.trace("vCenter API trace - getVmdkFileInfo() done");
			s_logger.trace("VMDK file descriptor: "
					+ new Gson().toJson(result.first()));
		}
		return result;
	}

	public void exportVm(String exportDir, String exportName,
			boolean packToOva, boolean leaveOvaFileOnly) throws Exception {
		ManagedObjectReference morOvf = _context.getServiceContent()
				.getOvfManager();
		...
		DatacenterMO dcMo = new DatacenterMO(_context,
				hostMo.getHyperHostDatacenter());

		if (runtimeInfo.getPowerState() != VirtualMachinePowerState.POWERED_OFF) {
			String msg = "Unable to export VM because it is not at powerdOff state. vmName: "
					+ vmName + ", host: " + hostName;
			s_logger.error(msg);
			throw new Exception(msg);
		}

		ManagedObjectReference morLease = _context.getService().exportVm(
				getMor());
		if (morLease == null) {
		...
		HttpNfcLeaseState state = leaseMo.waitState(new HttpNfcLeaseState[] {
				HttpNfcLeaseState.READY, HttpNfcLeaseState.ERROR });

		try {
			if (state == HttpNfcLeaseState.READY) {
				final HttpNfcLeaseMO.ProgressReporter progressReporter = leaseMo
				...
				try {
					...
					final long totalBytes = leaseInfo
							.getTotalDiskCapacityInKB() * 1024;
					long totalBytesDownloaded = 0;

					List<HttpNfcLeaseDeviceUrl> deviceUrls = leaseInfo
							.getDeviceUrl();
					s_logger.info("volss: copy vmdk and ovf file starts "
					...
					if (deviceUrls != null) {
						...
						for (int i = 0; i < deviceUrls.size(); i++) {
							...
							String orgDiskFileName = deviceUrlStr
									.substring(deviceUrlStr.lastIndexOf("/") + 1);
							String diskFileName = String.format("%s-disk%d%s",
									exportName, i, VmwareHelper
											.getFileExtension(orgDiskFileName,
													".vmdk"));
							String diskUrlStr = deviceUrlStr.replace("*",
									hostName);
							diskUrlStr = HypervisorHostHelper
									.resolveHostNameInUrl(dcMo, diskUrlStr);
							String diskLocalPath = exportDir + File.separator
									+ diskFileName;
							fileNames.add(diskLocalPath);

							if (s_logger.isInfoEnabled()) {
								s_logger.info("Download VMDK file for export. url: "
										+ deviceUrlStr);
							}
							long lengthOfDiskFile = _context.downloadVmdkFile(
									diskUrlStr, diskLocalPath,
									totalBytesDownloaded,
									new ActionDelegate<Long>() {
										public void action(Long param) {
											if (s_logger.isTraceEnabled()) {
												s_logger.trace("Download progress "
														+ param
														+ "/"
														+ totalBytes);
											}
											progressReporter
													.reportProgress((int) (param * 100 / totalBytes));
										}
									});
						...
						ovfDescParams.getOvfFiles().addAll(
								Arrays.asList(ovfFiles));
						OvfCreateDescriptorResult ovfCreateDescriptorResult = _context
								.getService().createDescriptor(morOvf,
										getMor(), ovfDescParams);
						String ovfPath = exportDir + File.separator
								+ exportName + ".ovf";
						...
						if (packToOva) {
							...
							Script command = new Script(false, "tar", 0,
									s_logger);
							...
							s_logger.info("Package OVA with commmand: "
									+ command.toString());
							command.execute();

							// to be safe, physically test existence of the
							// target OVA file
							if ((new File(exportDir + File.separator
									+ exportName + ".ova")).exists()) {
								success = true;
							} else {
								s_logger.error(exportDir + File.separator
										+ exportName
					...
					s_logger.info("volss: copy vmdk and ovf file finishes "
							+ System.currentTimeMillis());
				} catch (Throwable e) {
					s_logger.error("Unexpected exception ", e);
				} finally {
					...
					if (!success)
						throw new Exception(
								"Unable to finish the whole process to package as a OVA file");
				}
			}
		} finally {
	...
	public void setSnapshotDirectory(String snapshotDir) throws Exception {
		VirtualMachineFileInfo fileInfo = getFileInfo();
		Pair<DatacenterMO, String> dcInfo = getOwnerDatacenter();
		String vmxUrl = _context.composeDatastoreBrowseUrl(dcInfo.second(),
				fileInfo.getVmPathName());
		...
		try {
			in = new BufferedReader(new InputStreamReader(
					new ByteArrayInputStream(vmxContent)));
	...
	public void backupCurrentSnapshot(String deviceName,
			ManagedObjectReference morDestDs, String destDsDirectory,
		...
		List<Pair<ManagedObjectReference, String>> mounts = hostMo
		...
		for (int i = 0; i < snapshotInfo.length; i++) {
			...
			if (disks != null) {
				...
				for (SnapshotDescriptor.DiskInfo disk : disks) {
					if (deviceName == null
							|| deviceName.equals(disk.getDeviceName())) {
						String srcVmdkFullDsPath = getSnapshotDiskFileDatastorePath(
								vmFileInfo, mounts, disk.getDiskFileName());
						...
						String srcVmdkBaseFilePath = DatastoreFile
								.getCompanionDatastorePath(srcVmdkFullDsPath,
										vmdkInfo.first().getBaseFileName());

						destFileName = destName + (snapshotInfo.length - i - 1)
								+ ".vmdk";
						if (vmdkInfo.first().getParentFileName() != null) {
							destBaseFileName = destName
									+ (snapshotInfo.length - i - 1)
									+ "-delta.vmdk";
							destParentFileName = destName
									+ (snapshotInfo.length - i - 2) + ".vmdk";
						} else {
							destBaseFileName = destName
									+ (snapshotInfo.length - i - 1)
									+ "-flat.vmdk";
						...
						s_logger.info("Copy VMDK base file "
								+ srcVmdkBaseFilePath + " to "
								+ destDsDirectory + "/" + destBaseFileName);
						srcDsInfo.first().copyDatastoreFile(
								srcVmdkBaseFilePath, dcMo.getMor(), morDestDs,
								destDsDirectory + "/" + destBaseFileName,
								dcMo.getMor(), true);

						byte[] newVmdkContent = VmdkFileDescriptor
								.changeVmdkContentBaseInfo(vmdkInfo.second(),
										destBaseFileName, destParentFileName);
						String vmdkUploadUrl = getContext()
								.composeDatastoreBrowseUrl(dcMo.getName(),
										destDsDirectory + "/" + destFileName);

						s_logger.info("Upload VMDK content file to "
								+ destDsDirectory + "/" + destFileName);
						getContext().uploadResourceContent(vmdkUploadUrl,
								newVmdkContent);

						backupInfo.add(new Ternary<String, String, String>(
								destFileName, destBaseFileName,
								destParentFileName));
					}
				}
			}
		...
		byte[] vdiskInfo = VmwareHelper.composeDiskInfo(backupInfo,
				snapshotInfo.length, includeBase);
		String vdiskUploadUrl = getContext().composeDatastoreBrowseUrl(
				dcMo.getName(), destDsDirectory + "/" + destName + ".vdisk");
		getContext().uploadResourceContent(vdiskUploadUrl, vdiskInfo);
	}

	public String[] getCurrentSnapshotDiskChainDatastorePaths(String diskDevice)
			throws Exception {
		HostMO hostMo = getRunningHost();
		List<Pair<ManagedObjectReference, String>> mounts = hostMo
				.getDatastoreMountsOnHost();
		...
		for (int i = 0; i < snapshotInfo.length; i++) {
			...
			if (disks != null) {
				for (SnapshotDescriptor.DiskInfo disk : disks) {
					...
					if (diskDevice == null
							|| diskDevice.equalsIgnoreCase(deviceNameInDisk)) {
	...
	public Pair<VirtualMachineMO, String[]> cloneFromCurrentSnapshot(
			String clonedVmName, int cpuSpeedMHz, int memoryMb,
			String diskDevice, ManagedObjectReference morDs) throws Exception {
		assert (morDs != null);
	...
	public VirtualMachineMO cloneFromDiskChain(String clonedVmName,
			int cpuSpeedMHz, int memoryMb, String[] disks,
			ManagedObjectReference morDs) throws Exception {
		assert (disks != null);
		...
		VirtualMachineMO clonedVmMo = HypervisorHostHelper.createWorkerVM(
				hostMo, new DatastoreMO(hostMo.getContext(), morDs),
				clonedVmName);
		if (clonedVmMo == null)
	...
	public GuestOsDescriptor getGuestOsDescriptor(String guestOsId)
			throws Exception {
		GuestOsDescriptor guestOsDescriptor = null;
		if (guestOsId == null) {
			guestOsId = getGuestId();
		}
		ManagedObjectReference vmEnvironmentBrowser = _context.getVimClient()
				.getMoRefProp(_mor, "environmentBrowser");
		VirtualMachineConfigOption vmConfigOption = _context.getService()
				.queryConfigOption(vmEnvironmentBrowser, null, null);
		List<GuestOsDescriptor> guestDescriptors = vmConfigOption
	...
	public void copyAllVmDiskFiles(DatastoreMO destDsMo, String destDsDir,
			boolean followDiskChain) throws Exception {
		VirtualDevice[] disks = getAllDiskDevice();
		DatacenterMO dcMo = getOwnerDatacenter().first();
		if (disks != null) {
			for (VirtualDevice disk : disks) {
				List<Pair<String, ManagedObjectReference>> vmdkFiles = getDiskDatastorePathChain(
						(VirtualDisk) disk, followDiskChain);
				for (Pair<String, ManagedObjectReference> fileItem : vmdkFiles) {
					DatastoreMO srcDsMo = new DatastoreMO(_context,
							fileItem.second());

					DatastoreFile srcFile = new DatastoreFile(fileItem.first());
					DatastoreFile destFile = new DatastoreFile(
							destDsMo.getName(), destDsDir,
							srcFile.getFileName());
					...
					s_logger.info("Copy VM disk file " + srcFile.getPath()
							+ " to " + destFile.getPath());
					srcDsMo.copyDatastoreFile(fileItem.first(), dcMo.getMor(),
							destDsMo.getMor(), destFile.getPath(),
							dcMo.getMor(), true);

					if (vmdkDescriptor != null) {
						String vmdkBaseFileName = vmdkDescriptor.first()
								.getBaseFileName();
						String baseFilePath = srcFile
								.getCompanionPath(vmdkBaseFileName);
						destFile = new DatastoreFile(destDsMo.getName(),
								destDsDir, vmdkBaseFileName);

						s_logger.info("Copy VM disk file " + baseFilePath
								+ " to " + destFile.getPath());
						srcDsMo.copyDatastoreFile(baseFilePath, dcMo.getMor(),
								destDsMo.getMor(), destFile.getPath(),
								dcMo.getMor(), true);
					}
				}
	...
	public List<String> getVmdkFileBaseNames() throws Exception {
		...
		for (VirtualDevice device : devices) {
			if (device instanceof VirtualDisk) {
				vmdkFileBaseNames
						.add(getVmdkFileBaseName((VirtualDisk) device));
			}
		}
	...
	public String getVmdkFileBaseName(VirtualDisk disk) throws Exception {
		...
		if (backingInfo instanceof VirtualDiskFlatVer2BackingInfo) {
			...
			DatastoreFile dsBackingFile = new DatastoreFile(
	...
	@Deprecated
	public void moveAllVmDiskFiles(DatastoreMO destDsMo, String destDsDir,
			boolean followDiskChain) throws Exception {
		VirtualDevice[] disks = getAllDiskDevice();
		DatacenterMO dcMo = getOwnerDatacenter().first();
		if (disks != null) {
			for (VirtualDevice disk : disks) {
				...
				for (Pair<String, ManagedObjectReference> fileItem : vmdkFiles) {
					...
					s_logger.info("Move VM disk file " + srcFile.getPath()
							+ " to " + destFile.getPath());
					...
					if (vmdkDescriptor != null) {
						...
						s_logger.info("Move VM disk file " + baseFilePath
								+ " to " + destFile.getPath());
	...
	public Pair<VirtualDisk, String> getDiskDevice(String vmdkDatastorePath,
		...
		String trimmedSrcBaseName = VmwareHelper
				.trimSnapshotDeltaPostfix(srcBaseName);
		...
		if (matchExactly) {
			...
			s_logger.info("Look for disk device info from volume : "
					+ vmdkDatastorePath + " with trimmed base name: "
					+ trimmedSrcBaseName);
		}
			...
			for (VirtualDevice device : devices) {
				if (device instanceof VirtualDisk) {
					s_logger.info("Test against disk device, controller key: "
							+ device.getControllerKey() + ", unit number: "
							+ device.getUnitNumber());

					VirtualDeviceBackingInfo backingInfo = ((VirtualDisk) device)
							.getBacking();
					if (backingInfo instanceof VirtualDiskFlatVer2BackingInfo) {
						VirtualDiskFlatVer2BackingInfo diskBackingInfo = (VirtualDiskFlatVer2BackingInfo) backingInfo;
						do {
							...
							DatastoreFile dsBackingFile = new DatastoreFile(
									diskBackingInfo.getFileName());
							String backingBaseName = dsBackingFile
									.getFileBaseName();
							if (matchExactly) {
								if (backingBaseName
										.equalsIgnoreCase(srcBaseName)) {
									String deviceNumbering = getDeviceBusName(
											devices, device);

									s_logger.info("Disk backing : "
											+ diskBackingInfo.getFileName()
											+ " matches ==> " + deviceNumbering);
									return new Pair<VirtualDisk, String>(
											(VirtualDisk) device,
											deviceNumbering);
								}
							} else {
								if (backingBaseName
										.contains(trimmedSrcBaseName)) {
									String deviceNumbering = getDeviceBusName(
											devices, device);
			...
			for (VirtualDevice device : devices) {
				if (device instanceof VirtualDisk) {
					...
					if (backingInfo instanceof VirtualDiskFlatVer2BackingInfo) {
						...
						String deviceNumbering = getDeviceBusName(devices,
								device);
	...
	public VirtualDisk getDiskDeviceByDeviceBusName(String deviceBusName)
			throws Exception {
		List<VirtualDevice> devices = _context.getVimClient()
				.getDynamicProperty(_mor, "config.hardware.device");
			...
			for (VirtualDevice device : devices) {
				if (device instanceof VirtualDisk) {
					...
					if (backingInfo instanceof VirtualDiskFlatVer2BackingInfo) {
						...
						while (diskBackingInfo != null) {
							String deviceBusName = getDeviceBusName(devices,
									device);
							builder.addDisk(deviceBusName,
									diskBackingInfo.getFileName());
							diskBackingInfo = diskBackingInfo.getParent();
						}
					}
				...
				if (device instanceof VirtualDisk) {
					...
					if (backingInfo instanceof VirtualDiskFlatVer2BackingInfo) {
						...
						disks.add(new Pair<Integer, ManagedObjectReference>(
								new Integer(device.getKey()), diskBackingInfo
										.getDatastore()));
					}
				}
			}
	...
	@Deprecated
	public List<Pair<String, ManagedObjectReference>> getDiskDatastorePathChain(
			VirtualDisk disk, boolean followChain) throws Exception {
		VirtualDeviceBackingInfo backingInfo = disk.getBacking();
		if (!(backingInfo instanceof VirtualDiskFlatVer2BackingInfo)) {
		...
		if (!followChain) {
			pathList.add(new Pair<String, ManagedObjectReference>(
					diskBackingInfo.getFileName(), diskBackingInfo
							.getDatastore()));
			return pathList;
		}
		...
		DatastoreFile snapshotDirFile = new DatastoreFile(
				vmFilesInfo.getSnapshotDirectory());
		DatastoreFile vmxDirFile = new DatastoreFile(
				vmFilesInfo.getVmPathName());
		...
		do {
			if (diskBackingInfo.getParent() != null) {
				...
				try {
					String url = getContext().composeDatastoreBrowseUrl(
							dcPair.second(), diskBackingInfo.getFileName());
					...
					DatastoreFile currentFile = new DatastoreFile(
							diskBackingInfo.getFileName());
					String vmdkFullDsPath = snapshotDirFile
							.getCompanionPath(currentFile.getFileName());

					String url = getContext().composeDatastoreBrowseUrl(
							dcPair.second(), vmdkFullDsPath);
					content = getContext().getResourceContent(url);
					if (content == null || content.length == 0) {
						break;
					}
					...
					pathList.add(new Pair<String, ManagedObjectReference>(
							vmdkFullDsPath, diskBackingInfo.getDatastore()));
				}

				VmdkFileDescriptor descriptor = new VmdkFileDescriptor();
				descriptor.parse(content);
				if (descriptor.getParentFileName() != null
						&& !descriptor.getParentFileName().isEmpty()) {
					// create a fake one
					VirtualDiskFlatVer2BackingInfo parentDiskBackingInfo = new VirtualDiskFlatVer2BackingInfo();
					parentDiskBackingInfo.setDatastore(diskBackingInfo
							.getDatastore());
					...
					if (parentFileName.startsWith("/")) {
						...
						parentFileName = parentFileName
								.substring(fileNameStartPos + 1);
						parentDiskBackingInfo.setFileName(vmxDirFile
								.getCompanionPath(parentFileName));
					} else {
						parentDiskBackingInfo.setFileName(snapshotDirFile
								.getCompanionPath(parentFileName));
					}
	...
	public List<String> detachAllDisksExcept(String vmdkBaseName,
			String deviceBusName) throws Exception {
		List<VirtualDevice> devices = _context.getVimClient()
				.getDynamicProperty(_mor, "config.hardware.device");
		...
		for (VirtualDevice device : devices) {
			if (device instanceof VirtualDisk) {
				...
				VirtualDiskFlatVer2BackingInfo diskBackingInfo = (VirtualDiskFlatVer2BackingInfo) device
						.getBacking();
				...
				if (backingBaseName.equalsIgnoreCase(vmdkBaseName)
						|| (deviceBusName != null && deviceBusName
								.equals(deviceNumbering))) {
					continue;
				} else {
					s_logger.info("Detach " + diskBackingInfo.getFileName()
							+ " from " + getName());
	...
	public VirtualDisk getDiskDeviceByBusName(List<VirtualDevice> allDevices,
			String busName) throws Exception {
		for (VirtualDevice device : allDevices) {
			if (device instanceof VirtualDisk) {
	...
	public int getNextDeviceNumber(int controllerKey) throws Exception {
		...
		while (true) {
			// Next device number should be the lowest device number on the key
			// that is not in use and is not reserved.
		...
		if (sorted) {
			Collections.sort(nics, new Comparator<VirtualDevice>() {
				public int compare(VirtualDevice arg0, VirtualDevice arg1) {
					int unitNumber0 = arg0.getUnitNumber() != null ? arg0
							.getUnitNumber().intValue() : -1;
					int unitNumber1 = arg1.getUnitNumber() != null ? arg1
							.getUnitNumber().intValue() : -1;
	...
	public Pair<Integer, VirtualDevice> getNicDeviceIndex(
			String networkNamePrefix) throws Exception {
		List<VirtualDevice> nics = getNicDevices(true);
		...
		for (VirtualDevice nic : nics) {
			attachedNetworkSummary = ((VirtualEthernetCard) nic)
					.getDeviceInfo().getSummary();
			if (attachedNetworkSummary.startsWith(networkNamePrefix)) {
				return new Pair<Integer, VirtualDevice>(new Integer(index), nic);
			} else if (attachedNetworkSummary
					.endsWith("DistributedVirtualPortBackingInfo.summary")
				...
				if (dvPortGroupName != null
					...
					return new Pair<Integer, VirtualDevice>(new Integer(index),
							nic);
				}
			}
			index++;
	...
	public String getDvPortGroupName(VirtualEthernetCard nic) throws Exception {
		VirtualEthernetCardDistributedVirtualPortBackingInfo dvpBackingInfo = (VirtualEthernetCardDistributedVirtualPortBackingInfo) nic
				.getBacking();
		DistributedVirtualSwitchPortConnection dvsPort = dvpBackingInfo
				.getPort();
		...
		return (String) _context.getVimClient().getDynamicProperty(
				dvPortGroupMor, "name");
	}

	public VirtualDevice[] getMatchedDevices(Class<?>[] deviceClasses)
			throws Exception {
	...
	public void redoRegistration(ManagedObjectReference morHost)
			throws Exception {
		String vmName = getVmName();
		VirtualMachineFileInfo vmFileInfo = getFileInfo();
		...
		ManagedObjectReference morTask = _context.getService().registerVMTask(
				morFolder, vmFileInfo.getVmPathName(), vmName, false, morPool,
				hostMo.getMor());
		...
		if (!result) {
			...
			_context.waitForTaskProgressDone(morTask);
			if (isTemplate) {
				VirtualMachineMO vmNewRegistration = hostMo
						.findVmOnHyperHost(vmName);
	...
	public int getCoresPerSocket() throws Exception {
		...
		Integer coresPerSocket = (Integer) _context.getVimClient()
				.getDynamicProperty(_mor, "config.hardware.numCoresPerSocket");
	...
	private ManagedObjectReference getEnvironmentBrowser() throws Exception {
		if (_vmEnvironmentBrowser == null) {
			_vmEnvironmentBrowser = _context.getVimClient().getMoRefProp(_mor,
					"environmentBrowser");
		}
